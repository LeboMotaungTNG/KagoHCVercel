...
const CompanyDetailsTab = () => {
  ...
  const [companyData, setCompanyData] = useState({
    name: "Kago Human Capital", size: 45, sector: "Technology", email: "info@kagohc.com",
    phone: "+27 11 123 4567", alternativePhone: "+27 11 123 4568", website: "www.kagohc.com", fax: "+27 11 123 4569",
    taxId: "1234567890", registrationNumber: "2020/123456/07", vatNumber: "4123456789",
    uifReference: "UIF123456789", sdlReference: "SDL123456789", workInjuryFundRef: "WIF123456789",
    bank: { bankName: "First National Bank", accountName: "Kago Human Capital", accountNumber: "62812345678", branchCode: "250655", accountType: "Business Cheque", swiftCode: "FIRNZAJJ" },
    contacts: {
      ceo: { name: "Thabo Mbeki", email: "thabo@kagohc.com", phone: "+27 82 123 4567" },
      finance: { name: "Naledi Dlamini", email: "naledi@kagohc.com", phone: "+27 83 123 4567" },
      payroll: { name: "Peter Petersen", email: "peter@kagohc.com", phone: "+27 84 123 4567" }
    },
    address: { street: "123 Business Park", city: "Johannesburg", state: "Gauteng", country: "South Africa", postalCode: "2196", physicalAddress: "123 Business Park, Sandton, Johannesburg", postalAddress: "PO Box 1234, Sandton, 2196" },
    businessType: "Private Company", yearsInOperation: 8, sarsBranch: "Sandton",
    incomeTaxNumber: "9876543210", payeReference: "PAYE123456789", provisionalTaxpayer: true,
    taxComplianceStatus: "Compliant", language: "English", timezone: "Africa/Johannesburg",
    dateFormat: "DD/MM/YYYY", fiscalYearStart: "2025-03-01", fiscalYearEnd: "2026-02-28",
    status: "Active", verified: true, lastAuditDate: "2025-01-15"
  });
  ...
  useEffect(() => {
    const fetchCompanyData = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_URL}/owner/company/settings`, { headers: { "Authorization": `Bearer ${token}` } });
        const data = await response.json();
        if (data.success && data.data) {
          const mergedData = { ...companyData, ...data.data, bank: { ...companyData.bank, ...data.data.bank }, contacts: { ...companyData.contacts, ...data.data.contacts }, address: { ...companyData.address, ...data.data.address } };
          setCompanyData(mergedData); setFormData(mergedData);
        }
      } catch (error) { console.error("Error fetching company data:", error); } finally { setIsLoading(false); }
    };
    fetchCompanyData();
  }, []);
  ...
  const handleSave = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`${API_URL}/owner/company/settings`, { method: "PUT", headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify(formData) });
      if (response.ok) { setCompanyData(formData); setIsEditing(false); alert("Company details saved successfully!"); }
      else alert("Error saving company details");
    } catch (error) { console.error("Error:", error); alert("Error saving company details"); }
  };

  const getNestedValue = (obj: any, path: string): any => path.split('.').reduce((c, p) => c?.[p], obj);
  const setNestedValue = (obj: any, path: string, value: any): any => {
    const parts = path.split('.'); const newObj = { ...obj }; let current = newObj;
    for (let i = 0; i < parts.length - 1; i++) { current[parts[i]] = { ...current[parts[i]] }; current = current[parts[i]]; }
    current[parts[parts.length - 1]] = value; return newObj;
  };
  ...
  const renderField = (label: string, value: any, fieldPath: string, type: "text" | "email" | "number" | "date" | "checkbox" | "select" | "textarea" = "text") => {
    ...
    return (
      <Col md={6} key={fieldPath}>
        ...
        {isEditing ? (
          type === "textarea" ? <textarea rows={3} value={currentValue} onChange={(e) => setFormData(setNestedValue(formData, fieldPath, e.target.value))} style={{ width: "100%", padding: "8px 12px", borderRadius: "6px", border: "1px solid #CBD5E0", fontSize: "14px", resize: "vertical", minHeight: "80px" }} className="form-control" />
          : type === "number" ? <Input type="number" value={currentValue} onChange={(e) => setFormData(setNestedValue(formData, fieldPath, parseInt(e.target.value) || 0))} />
          : type === "date" ? <Input type="date" value={currentValue} onChange={(e) => setFormData(setNestedValue(formData, fieldPath, e.target.value))} />
          : type === "checkbox" ? <input type="checkbox" checked={currentValue === true || currentValue === "true"} onChange={(e) => setFormData(setNestedValue(formData, fieldPath, e.target.checked))} style={{ width: "auto", marginTop: "8px" }} className="form-check-input" />
          : type === "select" ? (
            <select value={currentValue} onChange={(e) => setFormData(setNestedValue(formData, fieldPath, e.target.value))} style={{ width: "100%", padding: "8px 12px", borderRadius: "6px", border: "1px solid #CBD5E0", fontSize: "14px" }} className="form-control">
              {label === "Business Type" && (<><option>Private Company (Pty Ltd)</option><option>Public Company (Ltd)</option><option>Non-Profit Organisation</option><option>Trust</option><option>Partnership</option><option>Sole Proprietorship</option></>)}
              {label === "Sector/Industry" && (<><option>Technology & Software</option><option>Finance & Banking</option><option>Healthcare & Medical</option><option>Manufacturing</option><option>Retail & E-commerce</option><option>Education & Training</option><option>Human Capital & Recruitment</option></>)}
              {label === "Language" && (<><option>English</option><option>Afrikaans</option><option>isiZulu</option><option>isiXhosa</option><option>Sepedi</option><option>Setswana</option><option>Other</option></>)}
              {label === "Timezone" && (<><option>Africa/Johannesburg</option><option>Africa/Cape Town</option><option>Africa/Durban</option></>)}
              {label === "Date Format" && (<><option>DD/MM/YYYY</option><option>MM/DD/YYYY</option><option>YYYY-MM-DD</option></>)}
              {label === "Account Type" && (<><option>Business Cheque</option><option>Business Savings</option><option>Business Current</option></>)}
              {label === "Status" && (<><option>Active</option><option>Inactive</option><option>Suspended</option><option>Pending Verification</option></>)}
              {label === "Bank Name" && (<><option>ABSA Bank</option><option>First National Bank (FNB)</option><option>Standard Bank</option><option>Nedbank</option><option>Capitec Bank</option><option>Discovery Bank</option></>)}
              {label === "SARS Branch" && (<><option>Sandton</option><option>Pretoria</option><option>Cape Town</option><option>Durban</option><option>Port Elizabeth</option></>)}
              {label === "Tax Compliance Status" && (<><option>Compliant</option><option>Pending</option><option>Non-Compliant</option><option>Under Review</option></>)}
            </select>
          ) : <Input type={type} value={currentValue} onChange={(e) => setFormData(setNestedValue(formData, fieldPath, e.target.value))} />
        ) : <p style={{ marginTop: "8px" }}>{type === "checkbox" ? (currentValue ? "Yes" : "No") : (currentValue || "-")}</p>}
      </Col>
    );
  };
  ...
  return (
    <div style={{ padding: "24px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
        ...
        {!isEditing ? <Button color="primary" onClick={() => setIsEditing(true)} style={{ backgroundColor: "#33A6CD", border: "none", borderRadius: "20px" }}><FaEdit size={14} style={{ marginRight: "8px" }} /> Edit Company</Button>
        : <Button color="success" onClick={handleSave} style={{ borderRadius: "20px" }}><FaSave size={14} style={{ marginRight: "8px" }} /> Save Changes</Button>}
      </div>
      <Card style={{ borderRadius: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.08)" }}>
        <CardBody>
          {isLoading && <div style={{ textAlign: "center", padding: "40px" }}><Spinner color="primary" /><p style={{ color: "#667085", marginTop: "12px" }}>Loading...</p></div>}
          {!isLoading && (
            <>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
                <div style={{ width: "120px", height: "120px", borderRadius: "50%", backgroundColor: "#33A6CD", display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: "48px", fontWeight: "bold" }}>{(companyData?.name || "K").charAt(0)}</div>
                {isEditing && <Button size="sm" style={{ borderColor: "#33A6CD", color: "#33A6CD" }}>Upload Logo</Button>}
              </div>
              <h5 style={{ marginBottom: "16px", color: "#33A6CD" }}>Basic Information</h5>
              <Row>{renderField("Company Name", companyData.name, "name")}{renderField("Company Size", companyData.size, "size", "number")}{renderField("Sector/Industry", companyData.sector, "sector", "select")}{renderField("Business Type", companyData.businessType, "businessType", "select")}{renderField("Years in Operation", companyData.yearsInOperation, "yearsInOperation", "number")}{renderField("Company Email", companyData.email, "email", "email")}{renderField("Phone Number", companyData.phone, "phone")}{renderField("Alternative Phone", companyData.alternativePhone, "alternativePhone")}{renderField("Fax Number", companyData.fax, "fax")}{renderField("Website", companyData.website, "website")}{renderField("Language", companyData.language, "language", "select")}{renderField("Timezone", companyData.timezone, "timezone", "select")}{renderField("Date Format", companyData.dateFormat, "dateFormat", "select")}</Row>
              <hr style={{ margin: "24px 0" }} />
              <h5 style={{ marginBottom: "16px", color: "#33A6CD" }}>Registration & Legal Information</h5>
              <Row>{renderField("Registration Number", companyData.registrationNumber, "registrationNumber")}{renderField("Tax ID / VAT Number", companyData.taxId, "taxId")}{renderField("VAT Number", companyData.vatNumber, "vatNumber")}{renderField("UIF Reference", companyData.uifReference, "uifReference")}{renderField("SDL Reference", companyData.sdlReference, "sdlReference")}{renderField("Work Injury Fund Ref", companyData.workInjuryFundRef, "workInjuryFundRef")}{renderField("Income Tax Number", companyData.incomeTaxNumber, "incomeTaxNumber")}{renderField("PAYE Reference", companyData.payeReference, "payeReference")}{renderField("SARS Branch", companyData.sarsBranch, "sarsBranch", "select")}{renderField("Provisional Taxpayer", companyData.provisionalTaxpayer, "provisionalTaxpayer", "checkbox")}{renderField("Tax Compliance Status", companyData.taxComplianceStatus, "taxComplianceStatus", "select")}</Row>
              <hr style={{ margin: "24px 0" }} />
              <h5 style={{ marginBottom: "16px", color: "#33A6CD" }}>Banking Information</h5>
              <Row>{renderField("Bank Name", companyData?.bank?.bankName || "", "bank.bankName", "select")}{renderField("Account Name", companyData?.bank?.accountName || "", "bank.accountName")}{renderField("Account Number", companyData?.bank?.accountNumber || "", "bank.accountNumber")}{renderField("Branch Code", companyData?.bank?.branchCode || "", "bank.branchCode")}{renderField("Account Type", companyData?.bank?.accountType || "", "bank.accountType", "select")}{renderField("SWIFT Code", companyData?.bank?.swiftCode || "", "bank.swiftCode")}</Row>
              <hr style={{ margin: "24px 0" }} />
              ...
              <Row>{renderField("CEO Name", companyData.contacts.ceo.name, "contacts.ceo.name")}{renderField("CEO Email", companyData.contacts.ceo.email, "contacts.ceo.email", "email")}{renderField("CEO Phone", companyData.contacts.ceo.phone, "contacts.ceo.phone")}</Row>
              <div style={{ marginTop: "12px" }} />
              <h6 style={{ fontSize: "14px", marginBottom: "12px", color: "#4A5568" }}>Finance Director / CFO</h6>
              <Row>{renderField("Finance Name", companyData.contacts.finance.name, "contacts.finance.name")}{renderField("Finance Email", companyData.contacts.finance.email, "contacts.finance.email", "email")}{renderField("Finance Phone", companyData.contacts.finance.phone, "contacts.finance.phone")}</Row>
              <div style={{ marginTop: "12px" }} />
              <h6 style={{ fontSize: "14px", marginBottom: "12px", color: "#4A5568" }}>Payroll Manager</h6>
              <Row>{renderField("Payroll Name", companyData.contacts.payroll.name, "contacts.payroll.name")}{renderField("Payroll Email", companyData.contacts.payroll.email, "contacts.payroll.email", "email")}{renderField("Payroll Phone", companyData.contacts.payroll.phone, "contacts.payroll.phone")}</Row>
              <hr style={{ margin: "24px 0" }} />
              <h5 style={{ marginBottom: "16px", color: "#33A6CD" }}>Address Information</h5>
              <Row>{renderField("Physical Address", companyData.address.physicalAddress, "address.physicalAddress", "textarea")}{renderField("Postal Address", companyData.address.postalAddress, "address.postalAddress", "textarea")}{renderField("Street", companyData.address.street, "address.street")}{renderField("City", companyData.address.city, "address.city")}{renderField("Province", companyData.address.state, "address.state")}{renderField("Country", companyData.address.country, "address.country")}{renderField("Postal Code", companyData.address.postalCode, "address.postalCode")}</Row>
              <hr style={{ margin: "24px 0" }} />
              <h5 style={{ marginBottom: "16px", color: "#33A6CD" }}>Fiscal Year & Status</h5>
              <Row>{renderField("Fiscal Year Start", companyData.fiscalYearStart, "fiscalYearStart", "date")}{renderField("Fiscal Year End", companyData.fiscalYearEnd, "fiscalYearEnd", "date")}{renderField("Company Status", companyData.status, "status", "select")}{renderField("Verification Status", companyData.verified, "verified", "checkbox")}{renderField("Last Audit Date", companyData.lastAuditDate, "lastAuditDate", "date")}</Row>
            </>
          )}
        </CardBody>
      </Card>
...
export const PayrollSettingsTab = () => {
  ...
  const generateStatReports = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // ✅ Key change: Get blob instead of JSON
        ...
        link.href = blobUrl;
        link.download = `EMP201_${activePayrollRun.period.replace(/ /g, "_")}.pdf`;
