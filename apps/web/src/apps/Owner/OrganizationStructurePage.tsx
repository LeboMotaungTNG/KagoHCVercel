...
// Company name/logo for the chart's root node come from the SAME module the
// Organization Settings page actually reads/writes through — NOT
...
function OrganizationStructureContent() {
  ...
  // Resolves after the browser has actually finished laying out and
  // painting the current frame (not just "next microtask") — needed
  // because html2canvas can otherwise snapshot mid-reflow right after we
  // resize the container above, producing broken/partial connector lines.
  const waitForPaint = () =>
  ...
  const captureChart = async (): Promise<HTMLCanvasElement | null> => {
    ...
    try {
      ...
      // Let the browser actually finish reflowing/painting the resized
      // layout — the connector lines are CSS ::before/::after borders
      // whose positions depend on final layout, and html2canvas is
      // sensitive to being invoked before that settles.
      await waitForPaint();
      ...
      const canvas = await html2canvas(outer, {
        ...
        scrollX: 0,
        scrollY: 0,
        // html2canvas clones the DOM into an offscreen document before
        // rendering; forcing a synchronous layout read there ensures the
        // clone's own pseudo-element geometry is fully resolved before
        // html2canvas walks the tree, which noticeably reduces missing/
        // partial connector lines versus capturing the clone as-is.
        onclone: (clonedDoc) => {
  ...
  const load = async () => {
    ...
    // Uses orgApi.loadCompanySettings() — the same call the Organization
    // Settings page itself uses to load/display the company name — rather
    // than onboarding.ts's fetchCompanySettings, which was hitting an
    // unrelated/unused endpoint.
    try {
      const res = await orgApi.loadCompanySettings();
